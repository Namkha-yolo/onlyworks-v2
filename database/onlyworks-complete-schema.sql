-- =============================================================================
-- ONLYWORKS COMPLETE DATABASE SCHEMA
-- Generated based on comprehensive codebase analysis
-- Includes ALL tables, columns, and relationships used by frontend/backend
-- =============================================================================
...
-- User settings table - separate for flexibility
...
-- Sessions table - core work session data
...
CREATE TABLE screenshots (
    ...
    analyzed BOOLEAN DEFAULT FALSE, -- Simplified version
...
-- Analysis batches table - for batch processing
CREATE TABLE analysis_batches (
    ...
    -- Extracted metrics for quick queries
...
-- Final analyses table - session-end summaries
CREATE TABLE final_analyses (
    ...
    session_summary TEXT, -- Simplified version
    key_achievements TEXT, -- Major accomplishments
    main_achievements TEXT, -- Simplified version
    ...
    improvement_suggestions JSONB DEFAULT '[]', -- Simplified version
...
-- Simplified analyses table (for unified access)
...
CREATE TABLE goals (
    ...
    sessions_count INTEGER DEFAULT 0, -- Simplified version
    productive_sessions INTEGER DEFAULT 0,
    total_time_minutes INTEGER DEFAULT 0, -- Simplified version
...
CREATE TABLE user_analytics (
    ...
    avg_productivity_score DECIMAL(3,1), -- Simplified alias
    average_focus_score DECIMAL(3,1),
    avg_focus_score DECIMAL(3,1), -- Simplified alias
...
INSERT INTO system_settings (key, value, description, category, is_public) VALUES
    ...
    ('schema_version', '"4.0.0"', 'Current database schema version', 'system', false),
    ('schema_applied_at', to_jsonb(NOW()::text), 'When the current schema was applied', 'system', false)
...
INSERT INTO system_info (key, value) VALUES
    ...
    ('features', '{"screenshots": true, "analysis": true, "goals": true, "teams": true, "analytics": true}')
ON CONFLICT (key) DO UPDATE SET
    value = EXCLUDED.value,
    updated_at = NOW();
...
-- User dashboard view (simplified)
...
SELECT
    ...
    u.name as user_name,
    EXTRACT(EPOCH FROM (COALESCE(s.end_time, NOW()) - s.start_time)) / 60 as duration_minutes,
    COUNT(ab.id) + COUNT(a.id) as analysis_count
FROM sessions s
JOIN users u ON s.user_id = u.id
LEFT JOIN analysis_batches ab ON s.id = ab.session_id
...
-- Success message
SELECT 'OnlyWorks Complete Database Schema v4.0 successfully applied! 🚀
All tables, indexes, triggers, and views created.
OAuth authentication will now work correctly.' as result;